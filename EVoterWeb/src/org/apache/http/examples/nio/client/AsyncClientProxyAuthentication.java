...
public class AsyncClientProxyAuthentication {
    ...
    public static void main(String[] args)throws Exception {
        ...
        credsProvider.setCredentials(
                new AuthScope("192.168.0.14", 1000),
                new UsernamePasswordCredentials("nvluong", "12345678"));
        CloseableHttpAsyncClient httpclient = HttpAsyncClients.custom()
                .setDefaultCredentialsProvider(credsProvider)
                .build();
        try {
            httpclient.start();
            HttpHost proxy = new HttpHost("192.168.0.14", 1000);
            RequestConfig config = RequestConfig.custom()
                    .setProxy(proxy)
                    .build();
            HttpGet httpget = new HttpGet("192.168.0.14");
