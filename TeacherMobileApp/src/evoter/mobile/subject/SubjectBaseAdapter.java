...
public class SubjectBaseAdapter extends BaseAdapter implements Filterable {
	...
	ArrayList<Subject> listFilterSubjects = new ArrayList<Subject>();
	LayoutInflater inflater;
	Context context;
	private ValueFilter valueFilter;

	public SubjectBaseAdapter(ArrayList<Subject> listSubjects, Context context) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		holder.subjectTitle = detail(convertView, R.id.tvSubname, listSubjects
	...
	private class ValueFilter extends Filter {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			if (constraint != null && constraint.length() > 0) {
				...
				for (int i = 0; i < listFilterSubjects.size(); i++) {
					if ((listFilterSubjects.get(i).getTitle().toUpperCase())
							.contains(constraint.toString().toUpperCase())) {
						Subject subject = new Subject(listFilterSubjects.get(i));
		...
		protected void publishResults(CharSequence constraint,
			...
			listSubjects = (ArrayList<Subject>) results.values;
	...
	public void deleteItem(long subjectID) {
		for (int i = 0; i < listSubjects.size(); i++) {
			if (listSubjects.get(i).getId() == subjectID) {
		...
		for (int i = 0; i < listFilterSubjects.size(); i++) {
			if (listFilterSubjects.get(i).getId() == subjectID) {
